...
- **Sticky Beskrivningstext + hours.** Picking a `Delproj` or `Aktivitet` makes Agresso re-render the row and overwrite the description with that entity's default text, so a row could only ever be filled in one order: codes first, text last. PRAgresso remembers what you typed and writes it back after each of those postbacks — write the description and punch the hours now, fill in the codes when you know them. Hours are only restored into cells Agresso blanked, never over a value it recalculated. Held fields get a blue left edge; clear a field to release the hold.
