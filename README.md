...
- **Delproj summary panel** — extra section above `Arbetstimmar` that groups every Tidtransaktion row by Delproj code and shows hours summed per group, so per-delprojekt totals are visible at a glance. Each row counts as `max(Sum, Fakt. värde)` (Sum used when Fakt. värde is empty; Fakt. värde used when it's higher than Sum). Collapsible via a double-arrow chevron in the panel header.
...
| Layout | Hide `Bereds.` / `Arb.typ`, Delproj label, Delproj summary panel, Arbetstimmar default-collapse | `hide_ace_code`, `hide_work_type`, `show_project_label`, `show_delproj_summary`, `arbetstimmar_collapsed_default` |
