(() => {
  ...
  const SETTING_DEFAULTS = {
    ...
    // Flag a lookup editor (Tidkod / Delproj / Aktivitet / ...) whose typed
    // value is not one the row's own menu offers. Aktivitet in particular is
    // scoped to the row's Delproj, so validity is per row, never a global
    // list. See installLookupValidation.
  ...
  // Reported from the field: a perfectly real Aktivitet code, typed on an
  // agreement that does not carry it, accepted without a murmur.
  //
  // Verified against the live grid. Typing `TEKX` into Aktivitet on a row
  // whose Delproj was already set, then tabbing out, leaves the editor
  // reading `TEKX`, writes `TEKX` into the hidden `$RowValue`, and renders
  // the cell exactly like every valid field — `getComputedStyle` reported
  // `rgb(255, 255, 255)` and the default grey border. Agresso says nothing
  // at all until the save fails.
  //
  // The tell sits in the other hidden sibling. `$RowDescription` holds the
  // resolved entity's name for a value that exists (`Normal arbetstid` for
  // Tidkod `N`) and the marker `[NY]` for one that does not.
  //
  // Which values exist is a per-row question, never a global one: Aktivitet
  // is scoped to the row's Delproj. Searching `T` with the Delproj cell
  // empty returned 51 activities; the identical search with a Delproj set
  // returned 2. So there is no catalogue to pre-bake and diff against — the
  // only authority is the list Agresso resolves for that row, which is
  // precisely what `$RowDescription` reports. No extra requests and no
  // replayed lookups: the answer is already in the DOM by the time the user
  // leaves the field.
  const LOOKUP_BAD_MARK = 'data-pragresso-badvalue';
  // `[NY]` is what this Swedish install renders; `[NEW]` is the English
  // equivalent. The generic bracket test catches any other localisation —
  // a real description would have to consist of nothing but a short
  // bracketed token to trip it, and being wrong costs a red ring, not data.
  const LOOKUP_NEW_MARKERS = ['[NY]', '[NEW]'];
  const LOOKUP_NEW_RE = /^\[[^\]]{1,12}\]$/;

  function isUnknownLookupValue(editor) {
    const typed = typeof editor.value === 'string' ? editor.value.trim() : '';
    if (!typed) return false;
    const desc = readEditorSibling(editor, 'RowDescription');
    if (desc) {
      if (LOOKUP_NEW_MARKERS.indexOf(desc.toUpperCase()) !== -1) return true;
      if (LOOKUP_NEW_RE.test(desc)) {
        logDebug('lookup: bracketed description treated as an unknown-value marker —', desc);
        return true;
      }
      return false;
    }
    // No description AND no resolved code: the editor holds text the server
    // never turned into anything. A resolved value with a genuinely empty
    // description still carries its code, so this cannot fire on one.
    return !readEditorSibling(editor, 'RowValue');
  }

  function applyLookupValidation() {
    const off = settings.flag_unknown_lookup_values === false;
    getAllDocuments().forEach((doc) => {
      if (!doc.querySelectorAll) return;
      if (off) {
        // Turning the option off clears the rings now, not on the next
        // postback.
        doc.querySelectorAll(`[${LOOKUP_BAD_MARK}]`).forEach((el) => el.removeAttribute(LOOKUP_BAD_MARK));
        return;
      }
      doc.querySelectorAll('tr.EditRow input.slcEditor').forEach((ed) => {
        // The field being typed in right now is mid-thought, not wrong:
        // `TE` on the way to `TEK` has no exact match either, and flashing
        // red on every keystroke trains the eye to ignore the colour.
        if (doc.activeElement === ed) return;
        if (isUnknownLookupValue(ed)) ed.setAttribute(LOOKUP_BAD_MARK, '1');
        else ed.removeAttribute(LOOKUP_BAD_MARK);
      });
    });
  }

  let _lookupValidationInstalled = false;
  function installLookupValidation() {
    if (_lookupValidationInstalled) return;
    _lookupValidationInstalled = true;

    const onFocusOut = (ev) => {
      const t = ev.target;
      if (!t || !t.classList || !t.classList.contains('slcEditor')) return;
      // Agresso's own blur handler (`ResetTextBox`, bound inline on every
      // editor) rewrites the editor and its hidden siblings. Reading them in
      // the same tick reads the values it is about to replace.
      setTimeout(() => { try { applyLookupValidation(); } catch (e) {} }, 0);
    };
    // Editing a flagged field clears the ring straight away instead of
    // leaving it red while the user fixes the value. The mark is re-derived
    // from the siblings the moment focus leaves, so an optimistic clear
    // costs nothing and stops the ring from contradicting the screen.
    const onInput = (ev) => {
      const t = ev.target;
      if (!t || !t.removeAttribute || !t.classList || !t.classList.contains('slcEditor')) return;
      t.removeAttribute(LOOKUP_BAD_MARK);
    };
