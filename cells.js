(() => {
  ...
  function readEditRowDaySum(row) {
    ...
    let any = false;
    for (let k = 1; k <= 7; ++k) {
      const inp = row.querySelector(`input[name$="$reg_value${k}$i"]`);
      if (!inp) continue;
      const n = parseAgressoNum(inp.value);
  ...
  function readEditRowDelprojCode(row) {
    ...
    // Title attribute carries the column label — stable across Agresso
    // versions for Swedish (Delproj). Fallbacks for English-localised
    // variants are cheap and don't false-match.
  ...
  function readEditRowDelprojName(row) {
    ...
    const base = editor.name.replace(/\$Editor$/, '');
    // RowDescription is a hidden sibling carrying the human-readable name.
    const desc = row.querySelector(`input[name="${base.replace(/[$]/g, '\\$')}$RowDescription"]`);
    return desc && typeof desc.value === 'string' ? desc.value.trim() : '';
  }
  function applyFaktVardeSum() {
    try {
      ...
      for (let i = 0; i < headers.length; ++i) {
        const txt = ((headers[i].textContent || '') + ' ' + (headers[i].getAttribute('title') || '')).trim();
        if (faktIdx < 0 && FAKT_HEADER_RE.test(txt)) { faktIdx = i; continue; }
        if (sumIdx < 0 && SUM_HEADER_RE.test(txt)) { sumIdx = i; }
      }
      if (faktIdx < 0) return;
  ...
  function recomputeDelprojSummary() {
    ...
    for (let i = 0; i < headers.length; ++i) {
      ...
      const isSum = SUM_HEADER_RE.test(tcText) || SUM_HEADER_RE.test(titleText);
      if (faktIdx < 0 && isFakt) { faktIdx = i; continue; }
      // The Sum column ALSO carries `data-fieldname^="reg_value"` (Agresso
      // names it `reg_value_sum` or similar), so a pure data-fieldname check
      // double-counts every row. Skip any reg_value column whose header text
      // is literally "Sum"/"Summa"/"Σ" — that's the aggregate, not a day.
      if (df && /^reg_value/i.test(df) && !isSum) {
        dayIdxs.push(i);
      }
    }
    ...
    for (const row of rows) {
      ...
      if (row.classList.contains('EditRow')) {
        ...
        if (!code) continue;
        name = readEditRowDelprojName(row);
  ...
  // semesterdagar".
  const SEMESTER_ROW_RE = /^(sparade dagar år\b|årets betalda semesterdagar\b)/i;
  const SALDO_DESC_RE = /beskrivning/i;
  const SALDO_URSPRUNG_RE = /ursprungligt/i;
  const SALDO_REST_RE = /restbelopp/i;
