...
### Added
- **A lookup value the row's own menu does not offer is now flagged red.** Reported from the field: a real `Aktivitet` code, typed on an agreement that does not carry it, accepted without a murmur. Reproduced against the live grid — typing `TEKX` into `Aktivitet` on a row whose `Delproj` was already set and tabbing out leaves the editor reading `TEKX`, writes `TEKX` into the hidden `$RowValue`, and renders the cell exactly like every valid field (`getComputedStyle` reported `rgb(255, 255, 255)` and the default grey border). Agresso raises no objection until the save fails.

  What makes this checkable at all is that Agresso has already done the work. Every lookup editor ships a hidden `$RowDescription` sibling holding the resolved entity's name for a value that exists (`Normal arbetstid` for Tidkod `N`) and the marker `[NY]` for one that does not. PRAgresso reads that on `focusout` and rings the field red. No extra requests, no replayed lookups, no picker opened behind the user's back.

  There is deliberately **no pre-baked list of valid codes**, because such a list cannot exist: `Aktivitet` is scoped to the row's `Delproj`. Measured on the live grid — the search `T` with the `Delproj` cell empty returned 51 activities; the identical search with a `Delproj` set returned 2. Validity is a per-row question and the only authority is the list Agresso resolves for that row.

  Details that matter for trusting it:
  - The field being typed in right now is never flagged. `TE` on the way to `TEK` has no exact match either, and a ring that flashes on every keystroke trains the eye to ignore the colour. Evaluation happens when focus leaves the editor.
  - Editing a flagged field clears the ring immediately rather than leaving it red while the value is being corrected; the mark is re-derived from the hidden siblings as soon as focus leaves again.
  - The check runs one tick after `focusout`. Agresso binds its own `ResetTextBox` inline on every editor and rewrites the editor and its siblings on blur, so reading in the same tick reads values it is about to replace.
  - `[NY]` is what a Swedish install renders and `[NEW]` its English equivalent; any other localisation is caught by a generic short-bracketed-token test. A description would have to consist of nothing but such a token to trip it, and being wrong costs a red ring, not data.
  - A red ring and the sticky feature's blue held-value edge can appear on the same field without either hiding the other.
  - New **Editing → Flag values the row's menu doesn't offer** option, on by default.

## 1.6.0 — 2026-08-21

### Added
- **The row you are editing now shows its Delproj name too.** Every static row carries the project name under its Delproj code, but the row in edit mode renders that column as a lookup editor with no `title` attribute — so the one row you are actually working on was the only row in the grid showing a bare code, and telling `123456-4` from `123401-1` apart meant leaving the row. The name is already in the DOM: each lookup editor ships hidden `$RowValue` / `$RowDescription` siblings, and the latter holds the human-readable name. PRAgresso now renders it under the editor with the same label element the static rows use (full text on hover, since the column ellipsizes at ~120px). It updates as soon as you pick a code, without waiting for a postback.

### Fixed
- **A held `Beskrivningstext` no longer jumps to another row when you delete a row.** Reported from the field as "it moves the text from the row I delete to the next object in the list, the one it jumps to when the row disappears" — and reproduced exactly against the live grid. Root cause: 1.5.4 keyed each hold on the row's editor-name head (`b$s89$g89s90$row3`), on the assumption that Agresso keeps that head stable across postbacks. It does not. `row<N>` is an **ordinal**, reassigned on every render in the grid's current sort order: adding a row inserts it in sort position and pushes everything after it down (verified — after `Lägg till` the existing `row0..row6` came back as `row1..row7`), and deleting one pulls everything after it up. So the hold recorded against the deleted row addressed whichever row inherited its number, and since Agresso puts that very row into edit mode after the delete, the text landed in it — and, being "seen" again, the hold then stayed there, rewriting itself after every subsequent postback. Three guards now: the `Ta bort` and `Lägg till` clicks release every hold *before* the postback carries the renumbering out; a column-header click does the same, because sorting reorders rows without changing how many there are; and each hold records the grid's row count when it was taken, so any pass that finds a different count drops the holds instead of writing them back. The row-count check waits for the grid to be fully in the DOM (footer row present, or the document past `loading`) so a pass landing mid-parse can't under-count and drop a hold that was perfectly good.
- **Delproj code and name were unreadable on a row whose Delproj had just been picked.** `readEditRowDelprojCode` / `readEditRowDelprojName` — used by the Delproj summary panel for the row in edit mode — anchored on `input[title="Delproj"]`, and Agresso *clears that title* the moment the user picks a value from the editor's autocomplete (verified live: the picked editor came back with `title=""` while the untouched Aktivitet editor beside it still had `title="Aktivitet"`). Both readers returned empty for exactly the rows that had a value to read, so a freshly categorised row silently dropped out of the panel's grouping. They now fall back to the column position — edit rows keep a 1:1 cell↔header alignment in this build, so the header carrying `data-fieldname="work_order"` / `"project"` finds the cell regardless of what the editor's attributes say — and read the code from the hidden `$RowValue` sibling when the editor itself renders blank.

## 1.5.4 — 2026-08-10

### Added
- **Typed `Beskrivningstext` and hours now survive a `Delproj` / `Aktivitet` change.** Picking either code fires a full postback that *replaces the entire grid document*, and the server re-renders the description from the picked entity's default text — the project name for Delproj, the activity name for Aktivitet. Anything you had written was gone, so the row could only be filled in one order: codes first, text last. Verified against the live grid: typing `TEST FOR CLAUDE` and then switching Aktivitet `UTB` → `ITS` came back as `IT-Strategi`. PRAgresso now records what you actually typed and writes it back into the freshly rendered row after every such postback, so you can write the description and punch the hours immediately and fill in the codes later, once you know them. Held fields carry a blue left edge; clearing a field releases the hold and lets Agresso's default text back in. New **Editing → Keep typed `Beskrivningstext` and hours** option, on by default.

  Details that matter for trusting it:
  - Only text a *human* typed is held. The capture path gates on `event.isTrusted`, so the server's own default text can never capture itself and pin the wrong string.
  - Hours are restored **only into a day cell the postback blanked or zeroed**. A non-zero value already in the cell is the server's own arithmetic (a `Dagar`→hours conversion on an absence row, say) and outranks the snapshot.
  - Restores set `.value` and Agresso's hidden `<field>$IsDirty` marker but deliberately dispatch **no** `input` / `change` event: Agresso binds its own `onchange` to these editors, and a synthetic one re-triggers the very postback being recovered from — an endless re-render loop. Form serialisation reads the property, so the value posts on the next real submit regardless.
  - State lives in `sessionStorage`, not a module variable, because the postback swaps the grid document out from under the content script and every module-level `let` is re-initialised on the other side. It is per-tab, dies with the tab, and entries expire after 6 h.
  - A held row is forgotten once it has been out of edit mode for 5 s (saved, deleted, or cancelled) — comfortably longer than a postback's ~300 ms dead window, so a row is never dropped mid-flight. Cleanup is gated on the timesheet grid actually being on screen, so no other Agresso page can clear the store.

## 1.5.3 — 2026-08-07

### Fixed
- **Delproj panel no longer drops a row's hours while that row is in edit mode.** Absence rows (e.g. Tidkod SEM with a Dagar unit) render their Delproj and Tidsenhet cells as locked `GridCell EditLabel` cells with *no editor input* when the row enters edit mode. The panel's edit-row readers only looked for editor inputs by title (`input[title="Delproj"]`), came back empty, and skipped the whole row — a vacation row's 24 h silently vanished from the summary (Σ 48.00 → 24.00) until the row went static again. Edit rows keep a 1:1 cell↔header alignment in this build (hidden columns included), so when the counts line up the readers now fall back to reading the locked cells by header index, reusing the static path's code extraction and Dagar→hours conversion.
- **Edit-row day sums no longer stop at seven day columns.** `readEditRowDaySum` looped `reg_value1..7`, but a period view bracketed by both adjacent weekends renders nine day columns (`reg_value1..9`) — days 8+ silently dropped out of an edit row's total. The reader now scans all `$reg_valueN$i` inputs by name suffix, matching how the static path already derives day columns from the headers.

## 1.5.2 — 2026-08-07

### Fixed
- **Beskrivningstext no longer paints across neighbouring columns.** Unit4's own `AgressoStyle.css` renders the timesheet grid with `table-layout: fixed`, and fixed layout ignores `min-width` on cells — so when horizontal space runs out, the description *column* can collapse far below our 500px floor while the extension simultaneously pinned the *inner* text `<div>` at `min-width: 500px`. The 500px-wide div inside a collapsed cell painted straight over Bereds./Arb.typ and Tidsenhet. The inner-div floor is gone (the td/th floor still applies in auto-layout grids, and in fixed layout the column flexes via the header's `width: 100%`), and description cells now clip with `overflow: hidden` instead of overlapping when the column genuinely runs out of room.
- **Adding a row no longer makes the page "jump" away from where you were.** After *Lägg till*, Agresso re-renders the grid and runs its own scroll-into-view; our rAF/debounced passes then re-inserted per-row project labels and re-rendered the Delproj summary panel above the grid. Every pixel of height those added above the first visible row pushed the page content down without `scrollTop` moving — reading as a scroll jump the user had to correct manually. Label and panel passes now run through a `withStableViewport` wrapper that anchors the first grid row intersecting the viewport top and restores its viewport-relative position afterwards. It is a strict no-op at the top of the page or when nothing above the anchor changed, so Agresso's own intentional scrolling is never fought.
